...
When a user prompt starts with `$revision-check`, use the `revision-check` approval workflow:
...
If the prompt does not start with `$revision-check`, use the normal project workflow.

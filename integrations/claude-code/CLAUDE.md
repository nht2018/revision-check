...
If the user's request begins with `$rev-check`, do not edit files immediately.
...
4. Start `skills/rev-check/scripts/serve_approval.py` to open the local approval page.
...
If the request does not begin with `$rev-check`, follow the normal Claude Code workflow.
